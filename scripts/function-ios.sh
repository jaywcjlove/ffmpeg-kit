...
display_help() {
  ...
  echo -e "\n'$COMMAND' builds FFmpegKit XCFrameworks for iOS. By default arm64 device and arm64/x86-64 simulator \
architectures are enabled without any external \
...
libraries are created under the prebuilt folder.\n"
  ...
  display_help_options "  -x, --xcframework\t\taccepted for compatibility; XCFramework output is always enabled" "  -l, --lts			build lts packages to support sdk 10+ devices" "      --target=ios sdk version\t\t\toverride minimum deployment target [12.1]"
  ...
  echo -e "  --disable-x86-64\t\tdo not build x86-64 simulator architecture [yes]\n"
...
create_ios_xcframeworks() {
  export ARCHITECTURE_VARIANT_ARRAY=("${ARCH_VAR_IPHONEOS}" "${ARCH_VAR_IPHONESIMULATOR}")
