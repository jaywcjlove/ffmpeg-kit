  ...
  *)
  ...
  if [[ ${SUPPORTED} -ne 1 ]]; then
    if [[ -z ${BUILD_FORCE} ]]; then
      echo -e "INFO: Disabled ${ARCH_NAME} architecture which is not supported by iOS SDK $DETECTED_IOS_SDK_VERSION\n" 1>>"${BASEDIR}"/build.log 2>&1
...
build_modulemap() {
  ...
  cat >"${FILE_PATH}" <<EOF
framework module ffmpegkitcore {
  umbrella "../Headers"
  export *
}
EOF
