...
display_help() {
  ...
  echo -e "\n'$COMMAND' builds FFmpegKit XCFrameworks for macOS. By default two architectures (arm64 and x86-64) are \
...
When compilation ends, libraries are created under the prebuilt folder.\n"
  ...
  display_help_options "  -x, --xcframework\t\taccepted for compatibility; XCFramework output is always enabled" "  -l, --lts			build lts packages to support sdk 10.12+ devices" "      --target=macos sdk version\toverride minimum deployment target [10.15]"
