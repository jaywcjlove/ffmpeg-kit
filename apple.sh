...
display_help() {
  ...
  echo -e "\n'$COMMAND' combines FFmpegKit frameworks into XCFrameworks for iOS and macOS. \
It uses frameworks created under the prebuilt folder for iPhone devices, iPhone simulators and macOS as input and builds XCFrameworks under \
the prebuilt folder.\n\nPlease note that this script is only responsible of packaging existing frameworks, created by \
'ios.sh' and 'macos.sh'. Running it will not compile any of these libraries again. Both top-level build scripts \
must be run with matching library options first. \
After that this script should be used to create an umbrella xcframework.\n"
  ...
  echo -e "  -l, --lts\t\t\tinclude lts packages to support iOS 10+ and macOS 10.12+ devices\n"
...
echo -e "INFO: Using iOS SDK: ${DETECTED_IOS_SDK_VERSION}, macOS SDK: ${DETECTED_MACOS_SDK_VERSION} by Xcode provided at $(xcode-select -p)\n" 1>>"${BASEDIR}"/build.log 2>&1
...
for run_arch_variant in ${ARCH_VAR_IPHONEOS} ${ARCH_VAR_IPHONESIMULATOR} ${ARCH_VAR_MACOS}; do
